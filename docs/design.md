...
- 固定的 Gemini 云端模型能够完成至少一个真实端到端 Diagnosis Run；
- 五个工具均通过统一契约测试；
...
flowchart LR
    ...
    Agent --> Model[Gemini ModelProvider]
...
| Model | 通过固定 HTTPS 协议请求 Gemini 阶段决策 | 模型加载、量化、GPU 生命周期和凭证持久化 |
...
真实模型不填写一个万能决策对象。各阶段使用字段较少的独立 Schema：
...
CodeRCA 只实现一个固定的 Gemini ModelProvider，通过 Gemini Developer API 的 OpenAI-compatible HTTPS 接口调用 `gemini-3.6-flash`。基础地址固定为 `https://generativelanguage.googleapis.com/v1beta/openai/`，凭证只从宿主环境变量 `GEMINI_API_KEY` 读取。Agent 进程不加载模型，也不管理量化、CUDA、显存或本地诊断 GPU。

真实后端使用文本请求和阶段级结构化 JSON 响应，不依赖模型直接执行工具。开发早期必须用最小探针验证：
...
自动化测试使用 FakeModelProvider。真实探针和 Agent Evaluation 必须显式启动；默认 CI 不读取 API Key、不联网，也不消费 Gemini 配额。MVP 不实现本地模型、其他云端提供商、多真实后端、自动路由或录制轨迹回放。

Gemini 响应中的提供商扩展字段不进入 Agent 领域状态。若响应包含 `extra_content.google.thought_signature`，Provider 只把它作为不透明元数据原样保留，并在继续同一提供商交互时按协议回传；不得解释、改写或将其转换为 Evidence、Observation 或私有思维链。

### 8.3 云端数据与凭证边界

真实模型请求只包含 8.1 定义的有界状态快照和当前阶段必要的代码片段。MVP 对云端数据处理采用保守边界：只允许向 Gemini 发送冻结公开基准仓库的内容，不支持私有仓库、雇主代码或其他敏感源代码。

`GEMINI_API_KEY` 不得写入 Prompt、Task Manifest、运行目录、事件、模型响应产物、日志、SQLite 或 Git，也不得作为 Tool 参数或环境变量传入 Docker。运行目录可以保存经过边界检查的公开任务 Prompt 与模型响应，以支持调试和 Evaluation。
...
embedding 在索引阶段预计算，reranker 在 CPU 上运行。诊断模型由 Gemini 云端提供，CodeRCA 的本地检索运行期不依赖 RTX 4060 Ti 或其他诊断 GPU。融合方法、候选数和 Top-K 在实现前冻结，不暴露给 Agent，也不做消融或在线调参。
...
- 只执行注册命令；
- 每次命令有统一超时；
- 不向容器挂载 `GEMINI_API_KEY` 或其他模型凭证；
- Gemini 请求由宿主 Agent 进程发起，不经过默认禁网的测试容器。
...
三个冻结任务显式运行，不进入默认 CI。默认 CI 使用 FakeModelProvider，不要求 API Key、GPU、真实模型或联网。
...
不得削减：最小状态机、五工具协议、真实 Gemini 模型调用、一个 Docker Experiment 和 Top-1 补丁 Validation。

## 17. 主要风险

| 风险 | 影响 | 控制 |
|---|---|---|
| Gemini 认证、配额、限流或网络失败 | Agent 无法真实闭环 | 运行前 preflight；结构化失败分类；真实任务显式运行 |
| 公开源码之外的数据被发送到云端 | 违反隐私边界 | 只允许冻结公开仓库；有界 Prompt；凭证与 Docker/产物隔离 |
...
| Gemini 输出不可应用补丁 | 核心验证失败 | 早期补丁探针；只允许一个小型 Top-1 补丁 |
...
- [ADR-0011：单一 Gemini 云端模型 API](adr/0011-single-gemini-cloud-model-provider.md)，取代 ADR-0009
